...
function selectedLineFitMode() {
  return document.querySelector('input[name="line-fit"]:checked')?.value || "uncertain";
}

function derivedLineFitClass(edges = displayedEdges) {
  const activation = new Set(ACTIVATION_EDGES.map(edgeKey));
  const visible = new Set(normalizedEdges(edges).map(edgeKey));
  const missingCount = [...activation].filter(key => !visible.has(key)).length;
  const extraCount = [...visible].filter(key => !activation.has(key)).length;
  if (!missingCount && !extraCount) return "normal_fit";
  if (!missingCount && extraCount) return "extra_lines";
  if (missingCount && !extraCount) return "missing_lines";
  return "mixed_lines";
}

function storedLineFitClass() {
  const mode = selectedLineFitMode();
  if (mode === "uncertain") return "uncertain";
  if (mode === "normal_fit") return "normal_fit";
  return derivedLineFitClass();
}
...
function previewPayload() {
  ...
  return {
    ...
    line_fit_class: storedLineFitClass(),
    line_fit_mode: selectedLineFitMode(),
...
function draw() {
  ...
  for (const [from, to] of ACTIVATION_EDGES) drawLine(from, to, "#ffd16680", true, 2);
...
function renderEdgeControls() {
  ...
  for (let a = 1; a <= 5; a += 1) {
    for (let b = a + 1; b <= 6; b += 1) {
      ...
      button.disabled = points.length !== 6 || selectedLineFitMode() === "normal_fit";
...
function renderTables() {
  ...
  const derivedFit = derivedLineFitClass();
  const fitLabels = {
    normal_fit: "Normal fit",
    extra_lines: "Extra lines",
    missing_lines: "Missing lines",
    mixed_lines: "Mixed changes",
    uncertain: "Uncertain"
  };
  $("#line-fit-derived").textContent = selectedLineFitMode() === "uncertain"
    ? `Stored classification: Uncertain. Current edge relationship: ${fitLabels[derivedFit]}.`
    : `Stored classification: ${fitLabels[storedLineFitClass()]}.`;
  ...
  copyActivationButton.disabled = points.length !== 6 || selectedLineFitMode() === "normal_fit";
  clearEdgesButton.disabled = displayedEdges.length === 0 || selectedLineFitMode() === "normal_fit";
...
function enforceNormalFitEdges() {
  if (selectedLineFitMode() === "normal_fit") {
    displayedEdges = points.length === 6 ? normalizedEdges(ACTIVATION_EDGES) : [];
  }
}

function render() {
  enforceNormalFitEdges();
  draw();
  renderEdgeControls();
  renderTagControls();
  renderTables();
...
async function loadMeasurement(id) {
  ...
  const savedClass = data.line_fit_class || "uncertain";
  const fitMode = savedClass === "normal_fit" || savedClass === "uncertain" ? savedClass : "custom";
  const lineFit = document.querySelector(`input[name="line-fit"][value="${fitMode}"]`);
...
canvas.addEventListener("click", event => {
  ...
  if (points.length === 6 && selectedLineFitMode() === "custom" && displayedEdges.length === 0) {
    displayedEdges = normalizedEdges(ACTIVATION_EDGES);
  }
  render();
});
...
for (const input of document.querySelectorAll('input[name="line-fit"]')) {
  input.addEventListener("change", () => {
    if (input.checked && input.value === "custom" && points.length === 6 && displayedEdges.length === 0) {
      displayedEdges = normalizedEdges(ACTIVATION_EDGES);
    }
    render();
  });
